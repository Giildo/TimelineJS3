...
export class TimeScale {
    ...
    constructor(timeline_config, options) {
        ...
        let slides = timeline_config.events;
    ...
    _computeDefaultSpan(timeline_config) {
        ...
        if (timeline_config.scale === 'human') {
            let formats = {}
            for (let i = 0; i < timeline_config.events.length; i++) {
                let fmt = timeline_config.events[i].start_date.findBestFormat();
                formats[fmt] = (formats[fmt]) ? formats[fmt] + 1 : 1;
            }

            for (let i = SCALES.length - 1; i >= 0; i--) {
                if (formats.hasOwnProperty(SCALES[i][0])) {
                    let scale = SCALES[SCALES.length - 1]; // default
                    if (SCALES[i + 1]) {
    ...
    */
    _computeRowInfo(positions, rows_left) {
        ...
        for (let i = 0; i < positions.length; i++) {
            ...
            for (let j = 0; j < lasts_in_row.length; j++) {
            ...
            if (typeof(pos_info.row) == 'undefined') {
                if (rows_left === null) {
                    ...
                    let min_overlap = Math.min.apply(null, overlaps);
                    let idx = overlaps.indexOf(min_overlap);
                    pos_info.row = idx;
    ...
    _computePositionInfo(slides, max_rows, default_marker_width) {
        ...
        for (let i = 0; i < slides.length; i++) {
            ...
            if (typeof(slides[i].end_date) != 'undefined') {
                let end_pos = this.getPosition(slides[i].end_date.getTime());
        ...
        if (!(groups.length)) {
            let result = this._computeRowInfo(this._positions, max_rows);
            ...
            for (let i = 0; i < this._positions.length; i++) {
                let pos_info = this._positions[i];

                pos_info.group = groups.indexOf(slides[i].group || "");
                pos_info.row = 0;

                let gi = group_info[pos_info.group];
                for (let j = gi.positions.length - 1; j >= 0; j--) {
            ...
            let n_rows = groups.length; // start with 1 row per group

            while (true) {
                // Count free rows available
                let rows_left = Math.max(0, max_rows - n_rows);
                if (!rows_left) {
                ...
                for (let i = 0; i < group_info.length; i++) {
                    ...
                    if (gi.n_overlaps && rows_left) {
                        let res = this._computeRowInfo(gi.positions, gi.n_rows + 1);
            ...
            for (let i = 0, row_offset = 0; i < group_info.length; i++) {
                this._group_labels.push({
                    label: group_info[i].label,
                    rows: group_info[i].n_rows
                });

                for (let j = 0; j < group_info[i].positions.length; j++) {
                    let pos_info = group_info[i].positions[j];
